...
## ✅ Закрыто 2026-06-30 — B3: тап-таргет стрелок витрины ≥44px (движок общий, эталон gruzia)
- [x] **[a11y/responsive]** Кликабельная зона стрелок карусели заведений `.scard__nav` (ShowcaseRail) увеличена до **≥44×44px** (WCAG 2.5.5, мобайл). Кнопка `<button>` теперь `min-width/min-height:44px`, контент по центру; видимая пилюля ~30px рисуется псевдоэлементом `::before` по центру тап-зоны, иконка над ней (`svg z-index:1`). Контраст-фикс a11y сохранён без изменений (slate 60% + белая обводка 40%, hover 78% — теперь на `::before`). Позиционирование скорректировано: `--prev/--next` смещены на `-7px`, чтобы видимая пилюля осталась у края медиа, а лишняя площадь тап-зоны ушла внутрь — стрелки не перекрывают карточку.

qa=GO · qa:responsive=GO (0 переполнений, 11 шаблонов × 5 ширин) · test:links=GO.
