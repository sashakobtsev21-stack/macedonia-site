...
- **Витрина (ShowcaseRail) на главной — наполнена.** `showcasePicks` в `src/layouts/HomePage.astro` = 3 опубликованных города (`things-to-do-in-skopje` / `things-to-do-in-ohrid` / `things-to-do-in-bitola`, `category: 'cities'`, kicker `city`, чип города Skopje/Ohrid/Bitola). Тип `showcasePicks` объявлен явным union `ShowcasePick[]` (вместо `as const`), чтобы петля-резолвер принимала будущие рестораны/маршруты/place-карточки. Золотая лента рендерится (в `dist/index.html` — карточки городов + kicker «City»).
