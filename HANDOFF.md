...
**Дата:** 2026-06-22 · **Этап:** контент-наполнение (Неделя 1 KALENDAR) — опубликованы статьи Скопье (столица), Охрид (якорь) и первые 2 новости. **Сайт переведён на один язык — только английский (en); ru/uk удалены.** UI-доводка главной: витрина наполнена городами + hero `ohrid-kaneo` ужат → `qa` = **GO**.
...
- **Витрина (ShowcaseRail) на главной — наполнена.** `showcasePicks` в `src/layouts/HomePage.astro` = 2 опубликованных города (`things-to-do-in-skopje` / `things-to-do-in-ohrid`, `category: 'cities'`, kicker `city`, чип города Skopje/Ohrid). Тип `showcasePicks` объявлен явным union `ShowcasePick[]` (вместо `as const`), чтобы петля-резолвер принимала будущие рестораны/маршруты/place-карточки. Золотая лента рендерится (в `dist/index.html` — карточки городов + kicker «City»).
- Гейты `check/build/test/links/lint` — **зелёные** (25 страниц). `npm run qa` = **GO** (0 критических / 0 средних; 29 минорных — пред-существующие декоративный-alt/CLS у hero-фона). Все генерируемые варианты изображений ≤200КБ.
