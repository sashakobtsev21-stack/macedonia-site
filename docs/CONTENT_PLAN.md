> ⚠️ **СПРАВОЧНИК, НЕ активный план (СУПЕРСЕДЕД 2026-06-22).** Единственный актуальный по-дневной контент-план — `/KALENDAR.md` (горизонт продлён до 31.07.2026, построен по SEO-аудиту). Этот файл оставлен ТОЛЬКО как тематическая справка: **кластерная карта pillar+spoke, интерлинк-схема (§3), балканские кросс-сетевые страницы (§4) и денежные связки** по каждому кластеру. **НЕ редактировать и НЕ читать как очередь публикаций** — она устарела (многое уже опубликовано, см. KALENDAR/PROGRESS), статусы/«ожидает O2/O4» здесь историчны.
>
> **Что устарело (не верить этому файлу):** очередь §5 «первых ~30» и пометки «ожидает O2/O4» — историчны. **O4 закрыт** (сайт EN-only, ru/uk удалены 2026-06-22): любые упоминания ru/uk-парности, пар en+uk и `check-parity` пары ниже — мёртвые, читать как «только en». Реальный порядок и статус публикаций — в `KALENDAR.md` (см. также опубликованные Охрид/Скопье/Битола/Lake Ohrid/Things-to-do-in-Ohrid в `PROGRESS.md`).
...
контент-план); фазы — ROADMAP.md (R0–R6, + архив-секция деталей). Процесс написания — CONTENT_GUIDE.md.
...
**Дата:** 2026-06-21 · **Статус:** СПРАВОЧНИК (суперсед KALENDAR.md; O4 закрыт — EN-only) · **Связано:** `STRATEGY.md` · `MACEDONIA-ANALYSIS-2026-06-21.md` · `ROADMAP.md` · `CONTENT_GUIDE.md`
...
- **Pillar + spoke.** В каждом кластере 1 опорная (pillar) страница-хаб + спицы. Спица ссылается на pillar и ≥1 соседа; pillar ссылается на все спицы (двусторонне; сайт одноязычный — en).
...
- **Правило:** каждая страница — ≥2 внутренние + ≥1 кросс-кластерная ссылка; pillar↔spoke двусторонне (сайт одноязычный — en).
...
## 5. Историческая стартовая очередь ~30 страниц (СУПЕРСЕДЕД — НЕ активна)

> ⚠️ Это **исходная гипотетическая очередь предзапуска (22.06)** — оставлена как карта кластеров/связок, НЕ как план публикаций. Реальный порядок и статус — `KALENDAR.md` (многие пункты ниже уже опубликованы: Охрид, Скопье, Битола, Lake Ohrid, Things to Do in Ohrid и др. — см. `PROGRESS.md`). Не отмечать здесь статусы и не вести по этому списку.

Каждая = **только en** (сайт одноязычный). MO несут AffiliateBox через `/go/`.
...
## 6. Открытые решения (историческая справка предзапуска)
- **O4 — RU-кластер при визе РФ:** ✅ **ЗАКРЫТО (2026-06-22): сайт одноязычный, только EN.** ru/uk удалены, парность ниже не применяется.
- **O2 — порядок входа язык×кластер:** на практике принят порядок «коммерция-first» (см. KALENDAR v3/v5: транспорт A→B → where-to-stay → сравнения → релокация P0 → money-planning → маршруты → еда → города → хвост).
- **Порог рейтинга директории еды** (≈≥4.3★ Google+TripAdvisor) + список источников — открыто, до §2.7 (R4).
- **O3 — взаимность кросс-сети:** подтверждены ли албанский/черногорский/хорватский сайты для ответных ссылок (нужно для §4) — открыто.
